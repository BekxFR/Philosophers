...
/*   Updated: 2023/01/04 17:38:57 by chillion         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_dead(t_p *philo)
{
	if (philo->data->dead == 1)
		return (0);
	if (philo->status != DEAD && philo->data->dead == 0)
	{
		if (philo->t_n_eat < ft_return_millisec())
		{
...
void	*ft_main_thread(void *arg)
{
	...
	var = (t_p *)arg;
	printf("var->t_n_die=%ld et P%d\n", var->t_n_eat, var->index);
	if (var->data->nbr_of_philo == 1)
		ft_usleep_milli(var, (var->t_die));
	while(var->status != DEAD && !var->data->dead)
	{
		if (!ft_check_dead(var))
			return (0);
		if (var->data->format == 1)
		{
			if (var->need_eat <= 0)
				return (0);
		}
		...
		if (var->status == EAT && ft_sleep(var) == -1)
			return (0);
		if (var->status == SLEEP && ft_think(var) == -1)
			return (0);
		if (var->status == THINK)
			var->status = WAIT;
	}
